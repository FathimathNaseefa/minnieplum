...
const wishlistAdd = async (req, res) => {
    ...
    try {
        ...
        if (user.wishlist.includes(productId)) {
            ...
            await user.save();
            return res.json({ success: true, status: "added", message: "Added to Wishlist" });
        }
    } catch (error) {
...
const getWishlist = async (req, res) => {
    try {
        ...
        // Populate wishlist with required fields including size and color
        const user = await User.findById(req.user._id)
            .populate({
                path: "wishlist",
                select: "productName productImage salePrice stock description size color"
            });
        ...
        console.log("✅ Wishlist Data Sent to Frontend:", JSON.stringify(user.wishlist, null, 2));
