...
const productDetails = async (req, res) => {
    try {
        ...
        const pdtDiscount = pdtOffer?.discountType === "percentage" ? pdtOffer.discountValue : 0;

        // Calculate category-specific discount
        const catDiscount = catOffer?.discountType === "percentage" ? catOffer.discountValue : 0;
        ...
        const discountPercent = Math.round(((product.salePrice - finalPrice) / product.salePrice) * 100);
...
//     try {
//         const userId = req.session.user;
//         const userData = userId ? await User.findById(userId) : null; // Ensure userData is never null
//         const productId = req.query.id;

//         // Fetch product and populate necessary fields
//         const product = await Product.findById(productId)
//             .populate("category")
...
//         // Ensure `pdtOffer` and `catOffer` are safely accessed
//         const pdtOffer = product.pdtOffer && product.pdtOffer.discountValue ? product.pdtOffer : null;
//         const catOffer = product.catOffer || null;

//         const pdtDiscount = pdtOffer ? (pdtOffer.discountType === "percentage" ? pdtOffer.discountValue : 0) : 0;
//         const catDiscount = catOffer?.discountType === "percentage" ? catOffer.discountValue : 0;
...
//             isWishlisted = userData.wishlist.includes(productId);
//         }
...
//         // Render the product details page
//         res.render("product-details", {
//             user: userData || {}, // Ensure user is always an object
//             product: {
//                 ...product.toObject(), // Convert Mongoose document to plain object
//                 finalPrice: finalPrice.toFixed(2), // Add finalPrice to the product object
//                 discountPercent, // Add discountPercent to the product object
